...
# Defines the main value column and its unique identifier for detail sheets
MAIN_VALUE_CONFIG = {
...
# Defines which columns should have totals in the detail sheets
    ...
    for invoice in exp_section.get("invoiceDetails", []):
        ...
        if not inv_array:
            entries.append({
                ...
                "Rate": "error", "Total Taxable Value": top_txval, "Integrated Tax": top_iamt,
        ...
        for inv_item_detail in inv_array:
            for item in inv_item_detail.get("itms", []):
                if not all(k in item for k in ("rt", "txval")): continue
                rt = item["rt"]
                txval = parse_number(item["txval"], float_2dec=True)
                iamt = parse_number(item.get("iamt", 0.00), float_2dec=True)
                camt = parse_number(item.get("camt", 0.00), float_2dec=True)
                samt = parse_number(item.get("samt", 0.00), float_2dec=True)
                csamt = parse_number(item.get("csamt", 0.00), float_2dec=True)
                entries.append({
                    "Invoice no": inum, "Invoice date": idt, "Reporting Month": reporting_month,
                    "GST payment": gst_payment, "Supply type": supply_type, "Total Invoice value": val,
...
def extract_hsn_entries(data_list):
    ...
    for data in data_list:
        ...
        if "fp" in data:
            ...
            period_key = list(data.keys())[0] if "month" not in data else \
                next((k for k in data.keys() if k != "month"), None)
    ...
    for note in cdnur_section.get("invoiceDetails", []):
        ...
        for item in itms:
            ...
            entries.append({
                ...
                "Computed Invoice Value": top_computed_value, "Taxable Value": txval,
                "Integrated Tax": iamt, "Central Tax": camt, "State/UT Tax": samt,
    ...
    for item in items:
        ...
        for doc_detail in item.get("docs", []):
            row = {
                ...
                "doc_type_title": doc_nature_title, "doc_type_code": f"DOC{doc_nature_code}"
            }
            results.append(row)
...
def extract_at_entries(data):
    ...
    for item in items:
        ...
        row = {
            "Month": reporting_month, "Place of Supply": item.get("pos", ""),
...
def extract_txpd_entries(data):
    ...
    if not txpd_obj: txpd_obj = data.get(period_key, {}).get("sections", {}).get("ATADJ", {})
    ...
    for buyer in data.get("b2b", []):
        ...
        for inv in buyer.get("inv", []):
            ...
            if not inv.get("itms"):
                ...
                row.update({
                    "Rate": "error", "Taxable Value": parse_number(inv.get("txval", 0), float_2dec=True),
            ...
            for item_detail_obj in inv.get("itms", []):
                ...
                row = invoice_base.copy()
                row.update({
                    "Rate": rate, "Taxable Value": parse_number(itm_det.get("txval", ""), float_2dec=True),
...
def _add_total_row_to_detail_sheet(ws, section_key, rows_data, column_headers,
    ...
    for row in rows_data:
        for col_header in columns_to_sum_and_display:  # Iterate only through columns that need totals
            ...
            if col_header == main_value_col_name and main_id_col_name:
                ...
                if doc_id_val:
                    ...
                    (col_header != main_value_col_name):  # Direct sum for other specified numeric columns
                total_row_values[col_header] += current_value_in_row
    ...
    for c_idx, col_header in enumerate(column_headers, 1):
        ...
        if not first_col_written:
            if c_idx == 1:
                cell.value = "Total"
                cell.font = RED_BOLD_FONT  # "Total" label is now red and bold
...
def _add_total_row_to_summary_sheet(ws, summary_data_list, display_columns, data_keys_map, format_map):
    ...
    for idx, col_name in enumerate(display_columns):
        if col_name == "Reporting Month":
            reporting_month_col_idx = idx + 1
            break
    if reporting_month_col_idx == -1:
        reporting_month_col_idx = 1
    ...
    for c_idx, display_col_name in enumerate(display_columns, 1):
        ...
        if c_idx == reporting_month_col_idx:
            ...
            data_key_to_fetch = display_col_name
            if display_col_name == "Note Value":
                data_key_to_fetch = "Invoice Value"

            if data_key_to_fetch in grand_totals:
                value = grand_totals[data_key_to_fetch]
                ...
                elif data_key_to_fetch in current_format_map_for_summary:
                    cell.number_format = current_format_map_for_summary[data_key_to_fetch]
            else:
                cell.value = ""


# Modified Excel Report Generation functions
def create_excel_report(data_dict, wb, ignore_warnings=False):
    ...
    for section_key, rows_data in data_dict.items():
        ...
        sheet_has_error = False  # Renamed from sheet_has_issue
        for r_idx, r_data in enumerate(rows_data, start=3):
            is_highlight = r_data.get("highlight", False)
            # Removed: if is_highlight: sheet_has_error = True (Highlight flag no longer makes tab red)
            for c_idx, col_name in enumerate(current_column_headers, start=1):
                val = r_data.get(col_name, "")
                cell = ws.cell(row=r_idx, column=c_idx, value=val)
                if val == "error":
                    sheet_has_error = True  # Tab turns red ONLY for "error" string
                    cell.font = RED_BOLD_FONT
                elif is_highlight:  # Row font still turns red for highlight
                    cell.font = RED_BOLD_FONT
        ...
        if sheet_has_error:  # Condition changed to only check for "error" string
...
def process_gstr1(small_files, large_files, excluded_sections_by_month, template_path, save_path,
    ...
    for file_path in small_files:
        ...
        for data_item in data_list_from_file:
            all_data_list_for_hsn.append(data_item)
            ...
            if "TXPD" not in current_exclusions: combined_data["TXPD"].extend(extract_txpd_entries(data_item))
    print("[DEBUG] Finished processing small JSON files")

    print("[DEBUG] Processing large JSON files...")
    for month_key, (filepath_large, large_file_excluded_sections) in large_files.items():
        ...
        for data_item_large in large_json_data_list:
            all_data_list_for_hsn.append(data_item_large)
            reporting_month_large_val = data_item_large.get("month", "Unknown")
            if reporting_month_large_val != "Unknown": processed_months.add(reporting_month_large_val)
    ...
    if not has_data_processed and not ignore_warnings:
        if not any(row for row_list in combined_data.get("DOC", []) for row in row_list):
            raise ValueError("No data found in provided JSON files for any section.")
    ...
    for section_sws_key in ["CDNR", "B2B,SEZ,DE"]:
        ...
        # numeric_cols_sws_check = NUMERIC_KEYS_BY_SECTION.get(section_sws_key, []) # Not needed for this check
        ...
        if not rows_sws_data or (not ignore_warnings and not has_meaningful_sws_data_check):
            ...
            continue
        sorted_rows_sws_data = sorted(rows_sws_data, key=lambda x_sws: (
            str(x_sws.get("Receiver Name", "")).strip().lower(),
            str(x_sws.get("GSTIN/UIN of Recipient", "")).strip().lower()))
        ...
        # Tab color red only if "error" string is present in any cell of this sheet's data
    ...
    summary_keys_for_summation = {col: col for col in summary_data_columns}
    ...
    sez_data_filter = [row for row in combined_data.get("B2B,SEZ,DE", []) if
                       row.get("Tax type") in ["SEZWP", "SEZWOP", "SEWP", "SEWOP", "SEZ"]]
    ...
    b2cs_summary = calculate_monthly_summary(combined_data.get("B2CS", []), "Reporting Month",
                                             value_key="Computed Invoice Value", taxable_key="Taxable Value",
                                             iamt_key="Integrated Tax", camt_key="Central Tax", samt_key="State/UT Tax",
                                             cess_key="Cess", invoice_key=None, processed_months=processed_months)
    ...
    if cdnr_summary and (ignore_warnings or any(
        ...
        fill_worksheet_data(ws_cdnr_sum, summary_data_columns, cdnr_summary)
    ...
    nil_summary = calculate_monthly_summary(combined_data.get("NIL", []), "Reporting Month",
                                            value_key="Computed Invoice Value", taxable_key="Computed Invoice Value",
                                            iamt_key=None, camt_key=None, samt_key=None, cess_key=None,
                                            invoice_key=None, processed_months=processed_months)
    if nil_summary and (ignore_warnings or any(
            r.get(k, 0) != 0 for r in nil_summary for k in ["Invoice Value", "Taxable Value"] if
            k != "No. of Records")):
        ws_nil_sum = create_or_replace_sheet(wb, "R1-Summary-NIL", SECTION_TITLES["Summary-NIL"], summary_data_columns)
        fill_worksheet_data(ws_nil_sum, summary_data_columns, nil_summary)
        _add_total_row_to_summary_sheet(ws_nil_sum, nil_summary, summary_data_columns, summary_keys_for_summation,
                                        COLUMN_FORMATS["Summary"])
    ...
    if doc_data_from_combined or processed_months:
        ...
        for row_doc_agg in doc_data_from_combined:
            ...
            temp_doc_agg[month_doc_agg]["net_issued"] += row_doc_agg["Net Issued"]
            temp_doc_agg[month_doc_agg]["total_num"] += row_doc_agg["Total Number"]
            temp_doc_agg[month_doc_agg]["cancelled"] += row_doc_agg["Cancelled"]
        ...
        doc_summary_data_list = [{"Reporting Month": month_iter_data, "No. of Records": values_data["doc_types_count"],
                                  "Net issued Documents": values_data["net_issued"],
                                  "Documents issued": values_data["total_num"],
                                  "Documents cancelled": values_data["cancelled"]} for month_iter_data, values_data in
                                 month_dict_doc.items()]
    ...
    b2ba_summary = calculate_monthly_summary(combined_data.get("B2BA", []), "Reporting Month",
                                             value_key="Total Invoice value", taxable_key="Total Taxable Value",
                                             iamt_key="Integrated Tax", camt_key="Central Tax", samt_key="State/UT Tax",
                                             cess_key="Cess", invoice_key="Revised/Original Invoice no",
    ...
    cdnur_b2cl_summary = calculate_monthly_summary(cdnur_b2cl_filter, "Reporting Month",
                                                   ...
                                                   samt_key="State/UT Tax", cess_key="Cess", invoice_key="C/D Note No",
                                                   processed_months=processed_months)
    if cdnur_b2cl_summary and (ignore_warnings or any(
        ...
        ws_cdnur_b2cl = create_or_replace_sheet(wb, "R1-Summary-CDNUR-B2CL", SECTION_TITLES["Summary-CDNUR-B2CL"],
                                                summary_display_columns_cdnr_cdnur)
        fill_worksheet_data(ws_cdnur_b2cl, summary_data_columns, cdnur_b2cl_summary)
    ...
    if cdnur_expwp_summary and (ignore_warnings or any(
        ...
        ws_cdnur_expwp = create_or_replace_sheet(wb, "R1-Summary-CDNUR-EXPWP", SECTION_TITLES["Summary-CDNUR-EXPWP"],
                                                 summary_display_columns_cdnr_cdnur)
        fill_worksheet_data(ws_cdnur_expwp, summary_data_columns, cdnur_expwp_summary)
    ...
    if cdnur_expwop_summary and (ignore_warnings or any(
        ...
        ws_cdnur_expwop = create_or_replace_sheet(wb, "R1-Summary-CDNUR-EXPWOP", SECTION_TITLES["Summary-CDNUR-EXPWOP"],
                                                  summary_display_columns_cdnr_cdnur)
        fill_worksheet_data(ws_cdnur_expwop, summary_data_columns, cdnur_expwop_summary)
    ...
    if cdnur_total_summary and (ignore_warnings or any(
        ...
        ws_cdnur_total = create_or_replace_sheet(wb, "R1-Summary-CDNUR-TOTAL", SECTION_TITLES["Summary-CDNUR-TOTAL"],
                                                 summary_display_columns_cdnr_cdnur)
        fill_worksheet_data(ws_cdnur_total, summary_data_columns, cdnur_total_summary)
    ...
    for ws_summary_iter in summary_ws_list_final:
        # Pass the actual data keys for formatting lookup if display names differ
        apply_format_and_autofit(ws_summary_iter, summary_data_columns,
...
if __name__ == '__main__':
    import traceback  # Ensure traceback is imported here as well

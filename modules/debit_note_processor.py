...
SECTION_TITLES = [
    ...
    ("DEBIT-Total_sws", "Debit Note Register - Total - Receiver wise"),
    ("DEBIT-Summary-Total", "Debit Note Register - Total - Summary"),
]

# Fixed headers (global for reference, same structure as Credit Note)
FIXED_HEADERS_LIST = [
...
def safe_float_conversion(value, header, cell=None, fixed_headers_ref=None, note_type="debit"):
    if fixed_headers_ref is None:
        fixed_headers_ref = FIXED_HEADERS_LIST
    ...
    if header in always_sign_specific_headers:
        ...
        else:  # For Debit Note, keep positive
            return abs(numeric_value)
...
def process_debit_note_data(input_files, template_file=None, existing_wb=None):
    ...
    fixed_headers = FIXED_HEADERS_LIST
    ...
    for filepath, branch_key in input_files:
        logging.info(f"Processing debit note file: {filepath}")
        try:
    ...
    for row in all_debit_note_data:
        for key in row.keys():
    ...
    logging.info(f"Final headers for debit note detail sheets: {final_headers}")
    ...
    all_debit_note_data.sort(key=sort_lambda)
    ...
    all_debit_note_data_sws = sorted(all_debit_note_data, key=sort_key_sws)
    ...
    for sheet_name_key, data_list_for_sheet in sheets_to_create:
        if data_list_for_sheet:
            ...
            logging.info(f"No data for debit note sheet: {sheet_name_key}, skipping.")

    summary_sheets_data_map = [
        ("DEBIT-Summary-Total", all_debit_note_data),
    ]
    ...
    for header in summary_headers:  # Ensure all relevant headers get formatting
    ...
    for sheet_name_key, data_for_summary in summary_sheets_data_map:
        if data_for_summary:
            ...
            logging.info(f"No data for debit note summary: {sheet_name_key}, skipping.")
    logging.info("Completed debit note data processing")
    return output_wb


class DebitNoteProcessorApp:
    def __init__(self, master):
        self.master = master
        master.title("Debit Note Processor")
        ...
        self.add_file_button = tk.Button(input_frame, text="Add Debit Note File(s)", command=self.add_files,
        ...
        self.process_button = tk.Button(master, text="Process Debit Note Data", command=self.process_data,
                                        font=button_font,
    ...
    def process_data(self):
        if not self.input_files:
            messagebox.showwarning("No Input Files", "Please add debit note files to process.")
            return
        ...
        try:
            ...
            result_wb = process_debit_note_data(self.input_files, template_file=None, existing_wb=existing_wb)

            if result_wb:
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                output_filename = os.path.join(self.output_dir, f"Processed_Debit_Note_Data_{timestamp}.xlsx")
                result_wb.save(output_filename)
                logging.info(f"Successfully processed debit note data and saved to {output_filename}")
                messagebox.showinfo("Processing Complete",
                                    f"Debit note data processed successfully!\nOutput saved to: {output_filename}")
                self.status_label.config(text="Processing complete!")
            else:
                logging.error("Processing returned no workbook for debit notes. Check logs.")
                messagebox.showerror("Processing Error", "An error occurred. Workbook not generated. Check logs.")
                self.status_label.config(text="Processing error.")
        except Exception as e:
            logging.error(f"An error occurred during debit note data processing: {e}", exc_info=True)
...
if __name__ == '__main__':
    root = tk.Tk()
    app = DebitNoteProcessorApp(root)
    root.mainloop()
